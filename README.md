...
## Штрихкоди (Open Food Facts)

Вкладка «Штрихкод» у щоденнику шукає пакетований продукт за штрихкодом через
відкриту базу [Open Food Facts](https://world.openfoodfacts.org) (ліцензія ODbL).
Код можна ввести вручну або відсканувати камерою (нативний `BarcodeDetector` —
Chrome/Edge; за відсутності підтримки лишається ручний ввід).

`GET /api/food/barcode/:code` проксує запит до OFF (`api/v2/product`), бере
поживність на 100 г (енергію переводить із кДж за потреби) і повертає **чернетку**
у тому ж форматі, що й `/api/food/recognize` — користувач підтверджує через
`POST /api/meals`. Відповіді OFF кешуються в памʼяті процесу на добу. Нічого не
імпортується масово: збережений запис стає звичайним `FoodItem` за назвою.
